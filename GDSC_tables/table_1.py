from path import relative_dir, table_1
import time

path = relative_dir+table_1

import pandas as pd
df = pd.read_csv(path)

df = df.melt(['TSPAN6'], var_name='Gene', value_name='GeneExpression')
...
result = interaction_permutation_gpu_batched(
    df,
    'TSPAN6',           # first factor column name
    'Gene',           # second factor column name
    'GeneExpression'    # numeric column name
)
